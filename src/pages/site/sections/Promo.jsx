...
export default function Promo() {
  return (
    <section style={{ background: 'linear-gradient(135deg,var(--blue-dkr),var(--blue) 50%,#1E63D6)', padding: '80px 24px', position: 'relative', overflow: 'hidden' }} aria-label="Promoção indique um amigo">
      ...
      <div className="site-container" style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: 48, position: 'relative', zIndex: 1 }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(245,162,0,.2)', border: '1px solid rgba(245,162,0,.35)', color: 'var(--amber)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.07em', padding: '5px 14px', borderRadius: 20, marginBottom: 18 }}>🎁 Promoção exclusiva</div>
        ...
        <div style={{ fontSize: 110, animation: 'float 4s ease-in-out infinite', flexShrink: 0 }} aria-hidden="true">🎁</div>
      </div>
    </section>
  )
}
